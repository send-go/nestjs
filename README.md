...
```typescript
import { SendgoModule, AccountClient } from '@sendgo/nestjs';
// 모듈의 imports에 등록하면 AccountClient를 주입할 수 있습니다.
SendgoModule.forAccount({ agentToken: process.env.SENDGO_AGENT_TOKEN! });
```
...
```ts
// 주입받은 SendgoService에 폴더 서비스가 공개됩니다.
await this.sendgo.templateFolders.list({ templateType: 'notice' });
```
